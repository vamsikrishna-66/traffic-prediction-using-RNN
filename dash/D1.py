...
dat = pd.read_excel("tt.xlsx",index_col=[0],skiprows={0})
    # dm = pd.DataFrame(mon)
fig = px.scatter(dat)
    #fig = mpl_to_plotly(fig)

mot = pd.read_excel("pp.xlsx",index_col=[0])
photo = px.bar(mot)


pot =  pd.read_excel("ttp.xlsx",index_col=[0])
palasa = px.bar(pot)
...
def update_output(value):
    ...
    mon = pd.read_excel("DAYWISEPERUNGUDI.xlsx",skiprows={0}, usecols= value,sheet_name="DAY1")
...
def update_output(value):
    ...
    day = pd.read_csv("velacheryday.csv",skiprows={0}, usecols=value)                                 
    # dm = pd.DataFrame(mon)
    fig = px.line(day)
    # fig = px.line(day["TUE"])
    fig.update_layout(
